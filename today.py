...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data, tv_time):
    ...
    justify_format(root, 'repo_data', repo_data, 33)
    justify_format(root, 'contrib_data', contrib_data)
    justify_format(root, 'follower_data', follower_data, 47)
    justify_format(root, 'loc_data', loc_data[2], 16)
    justify_format(root, 'loc_add', loc_data[0])
    justify_format(root, 'loc_del', loc_data[1], 7)
    if tv_time != '0 minutes':
        justify_format(root, 'tv_time', tv_time, 42)
...
if __name__ == '__main__':
    """
    Pranav Goyal (pranav174), 2022-2025
    """
    print('Calculation times:')
    # define global variable for owner ID and calculate user's creation date
    user_data, user_time = perf_counter(user_getter, USER_NAME)
    OWNER_ID, acc_date = user_data
    print(OWNER_ID, acc_date)
    formatter('account data', user_time)
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2000, 4, 17))
